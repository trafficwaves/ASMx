...
def calib(lane):
    ...
    model = AdaptiveSmoothing(kernel_time_window, kernel_space_window, dx, dt,
                              init_tau= 200.0, init_delta= 20.0, 
                              init_c_cong= 100.0, init_c_free= -400.5,
                              init_v_thr= 400.0, init_v_delta= 200.0).to(device)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        if epoch % 10 == 0 or epoch == 1:
            ...
            params_file = f'params_history_lane{lane}_explore.json'
