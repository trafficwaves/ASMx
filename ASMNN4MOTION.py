...
def main():
    ...
    for lane in range(1, 5):
        for date in dates:
            data = pd.read_csv(f'data/raw_data/motion/{date}/lane_{lane}_speed_matrix.csv', low_memory=False).values[:3600, :200].T
            print('data shape:', data.shape)
            speed = 3600*data.copy()
            # speed to be float32
            speed = speed.astype(np.float32)
            # speed = np.load('data/.npy')
            ...
            if not os.path.exists(f'data/processed_data/motion/lane{lane}'):
                os.makedirs(f'data/processed_data/motion/lane{lane}')
            np.save(f'data/processed_data/motion/lane{lane}/{date}.npy', sm)
